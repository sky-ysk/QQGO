...
### BUG-005: 好友上限 500 未实际测试  `fixed`

**描述：**
`MaxFriends = 500` 的业务规则已编码，但未进行边界测试（接近上限、等于上限、超过上限的情况）。

**修复：**
v0.7 补充 `TestFriendLimit500` 测试：499 好友 → 接受第 500 个 → 接受第 501 个失败 `ErrFriendLimit`。

**关联分支：** `feature/v0.3-friend` → `main` (v0.7)

---

### BUG-006: 好友离线请求通知未验证  `fixed`

**描述：**
当目标用户离线时，好友请求应持久化到 DB，用户上线后通过 `/friends` 可见。此流程未端到端测试。

**修复：**
v0.7 补充 `TestOfflineFriendRequest` 测试：A 发请求（B 离线）→ DB pending 记录 → B 查看待处理 → B 接受 → 双向好友。
