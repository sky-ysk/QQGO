...
**原因：**
客户端收到消息后直接 `fmt.Printf` 输出，但当前行可能已经有用户正在输入的内容或 prompt 前缀，导致输出位置错乱。需要在打印消息前先换行清除当前 prompt，打印后再重新显示 prompt。

**复现步骤：**
1. 终端 A 和 终端 B 都在线
2. 终端 A 给 终端 B 发消息 "hello"
3. 终端 B 收到消息，但显示格式混乱
...
### BUG-010: `/leavegroup` 后无法退出群聊窗口  `open`

**现象：**
在群组对话窗口中使用 `/leavegroup` 退出群后，客户端仍然停留在群聊模式。使用 `/to 10001` 切换到私聊后，虽然打印了历史消息，但发送消息时仍然被发到群组里，并且被服务端判定为非群组成员而拒绝。

**原因：**
`/leavegroup` 命令没有清除 `targetGroupID` 变量，导致客户端发送消息时仍然携带 `GroupID`。`/to` 命令切换后设置了 `targetQQ`，但没有清除 `targetGroupID`，导致两个目标同时存在，消息发送时 `GroupID` 优先于 `ToQQ`。

**复现步骤：**
1. 用户 A 创建群 G123，用户 B 加入
2. 用户 B: `/togroup G123` 切换到群聊
3. 用户 B: `/leavegroup G123` 退出群
4. 用户 B: `/to 10001` 切换到私聊
5. 用户 B 发送消息 → 消息仍发到群 G123 → 服务端返回 "not group member"
