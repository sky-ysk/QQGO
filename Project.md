...
| BUG | 说明 | 优先级 |
|-----|------|--------|
| BUG-009 | 接收方消息打印到 prompt 中间 | P0 |
| BUG-010 | `/leavegroup` 后无法退出群聊窗口 | P0 |

### P1 — 功能完善

| 需求 | 说明 |
|------|------|
| **群成员发送前校验** | 客户端群聊窗口发消息前校验成员身份，非成员自动退出窗口 |
| **群聊历史记录** | 群聊支持 `/prev` `/next` 翻页，`/togroup` 后自动拉取最近 30 条 |
| **本地聊天日志** | 客户端按 QQ 号分目录保存聊天记录到 `DATA/` 目录 |
| **Token 持久化客户端** | 登录成功后保存 token 到本地，下次启动自动登录 |
